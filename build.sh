...
readonly CONTAINER_NAME="GPU-Passthrough-on-NVIDIA-Jetson"
readonly PROJECT_DIRS=("diagnostics")
...
run_post_start_scripts() {
    ...
    if docker exec "${CONTAINER_NAME}" test -f "$onnx_script" 2>/dev/null; then
        ...
        if docker exec -it "${CONTAINER_NAME}" bash -c "$onnx_script --force"; then
            ...
            log_warning "GPU-Passthrough-on-NVIDIA-Jetson Enviroment Configuration - continuing anyway"
        fi
    else
...
connect_to_container() {
    ...
    echo -e "${GREEN}║    ./diagnostics → /advantech/diagnostics                      ║${NC}"
    echo -e "${GREEN}╚════════════════════════════════════════════════════════════════╝${NC}"
    echo
